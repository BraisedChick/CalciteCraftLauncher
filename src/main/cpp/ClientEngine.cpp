    ...
    try {
        switch (packetId) {
            ...
            case 0x38: { // Player Position And Look
                ...
                LOGI("DEBUG: glRenderer pointer = %p", glRenderer);
                
                // 如果渲染器已初始化，立即同步摄像机位置
                if (glRenderer) {
                    extern void syncCameraToPlayerPosition(double x, double y, double z, float yaw, float pitch);
                    LOGI("Calling syncCameraToPlayerPosition...");
                    syncCameraToPlayerPosition(playerX, playerY, playerZ, yaw, pitch);
                    LOGI("Camera automatically synced to player position");
                } else {
                    LOGI("glRenderer is null, cannot auto-sync camera");
                }
